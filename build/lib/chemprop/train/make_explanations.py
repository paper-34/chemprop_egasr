...
def make_explanations(args: ExplainArgs, smiles: List[List[str]] = None):
    ...
    exp_sign = np.sign(exp_preds-mu)
    print(mu, sigma)
    
    train_preds_norm = (train_preds - mu) / sigma
    
    # Explanation by E-GSAR
    def explain(graph_num, cont_embds, cont_preds, reg):
        cur_embds = exp_embds[graph_num]
        cur_sign = exp_sign[graph_num]
        
        cont_embds = [embds for embds in cont_embds.values()]

        w = []

        for i in range(cont_embds.shape[0]):
            cont_embd = cont_embds[i]
            ...
            w.append(-cur_sign[i] * cont_preds[i] * np.sum(ot.sinkhorn(x_weight, y_weight, M/M.max(), reg) * M, axis=1))
    ...
    if args.dataset_type == 'multiclass':
        raise ValueError('Now, E-GSAR is not supported to multi-class data')
    ...
    for i, ed in enumerate(exp_data):
        ...
        image.savefig(os.path.join(args.exp_path, smiles + '_explained.png'), bbox_inches='tight')
        image.clf()
