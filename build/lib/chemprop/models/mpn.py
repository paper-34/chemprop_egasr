...
class MPNEncoder(nn.Module):
    ...
    def get_node_embedding(self,
        ...
        :return: A PyTorch tensor of shape :code:`(num_atoms x (hidden + descriptor size))` containing the encoding of each atom for each molecule.
        """
        ...
        return atom_hiddens, a_scope  # num_atoms x (hidden + descriptor size) & start&end_index for each molecule
...
class MPN(nn.Module):
    ...
    def final_node_embedding(self,
        ...
        if self.atom_descriptors == 'descriptor':
            ...
            embeddings = [enc.get_node_embedding(ba, atom_descriptors_batch) for enc, ba in zip(self.encoder, batch)]
        else:
            embeddings = [enc.get_node_embedding(ba) for enc, ba in zip(self.encoder, batch)]
